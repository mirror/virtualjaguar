...
static uint16 tomPITPrescaler;
static uint16 tomPITDivider;
static uint16 tomPITPrescalerCounter;
static uint16 tomPITDividerCounter;
//uint32 tom_scanline;
//uint32 hblankWidthInPixels = 0;
uint16 tomIntControl;
uint16 tomIntPending;

//uint16 tom_jerry_int_pending, tom_timer_int_pending, tom_object_int_pending,
//	tom_gpu_int_pending, tom_video_int_pending;
//uint16 tom_int0_pending;
//uint16 tom_int1_pending;
//uint16 tom_int2_pending;
//uint16 tom_int3_pending;
//uint16 tom_int4_pending;
...
{
	// Tom's interrupt enable and pending bit are the same
	// so just optimizing the shifts
	uint16 bit = TOM_INT0_ENABLE << irqline;
	// Is interrupt enabled ?
	if (tomIntControl & bit)
	{
		// Set the latch bit
		//tomIntPending |= (TOM_INT0_PENDING << irqline);
		tomIntPending |= bit;

		//TOMHandleIRQs();	TOM has nothing to do

		// Generate the 68k interrupt
#ifdef TOM_DEBUG
		WriteLog("TOM: Generating 68k interrupt.\n");
#endif
		m68k_set_irq(7);
...
void tom_reset(void)
{
	...
	tomPITPrescaler = 0;
	tomPITDivider = 0;
	tomPITPrescalerCounter = tomPITPrescaler;
	tomPITDividerCounter = tomPITDivider;
...
//
uint8 TOMReadByte(uint32 address, uint32 who/*=UNKNOWN*/)
{
//???Is this needed???
...
//	offset &= 0xFF3FFF;
	...
	if ((address >= GPU_CONTROL_RAM_BASE) && (address < (GPU_CONTROL_RAM_BASE + 0x20)))
		return GPUReadByte(address, who);
	else if ((address >= GPU_WORK_RAM_BASE) && (address < (GPU_WORK_RAM_BASE + 0x1000)))
		return GPUReadByte(address, who);
	else if ((address >= 0xF00010) && (address < 0xF00028))
		return OPReadByte(address, who);
	else if ((address >= 0xF02200) && (address < 0xF022A0))
		return BlitterReadByte(address, who);
	else if (address == 0xF00050)
		return tomPITPrescaler >> 8;
	else if (address == 0xF00051)
		return tomPITPrescaler & 0xFF;
	else if (address == 0xF00052)
		return tomPITDivider >> 8;
	else if (address == 0xF00053)
		return tomPITDivider & 0xFF;

	return tom_ram_8[address & 0x3FFF];
}

//
// TOM word access (read)
//
uint16 TOMReadWord(uint32 address, uint32 who/*=UNKNOWN*/)
{
//???Is this needed???
//	offset &= 0xFF3FFF;
#ifdef TOM_DEBUG
...
if ((address >= 0xF02000) && (address <= 0xF020FF))
	...
	if (address == 0xF000E0)
	{
		//WriteLog("TOM: Interrupt pending status: $%04X \n", tomIntPending);
		return tomIntPending;
	}
//Shoud be handled by the jaguar main loop now... And it is! ;-)
/*	else if (address == 0xF00006)	// VC
...
// !!! FIX !!!
	else if (address == 0xF00004)
		return rand() & 0x03FF;
	else if ((address >= GPU_CONTROL_RAM_BASE) && (address < (GPU_CONTROL_RAM_BASE + 0x20)))
		return GPUReadWord(address, who);
	else if ((address >= GPU_WORK_RAM_BASE) && (address < (GPU_WORK_RAM_BASE + 0x1000)))
		return GPUReadWord(address, who);
	else if ((address >= 0xF00010) && (address < 0xF00028))
		return OPReadWord(address, who);
	else if ((address >= 0xF02200) && (address < 0xF022A0))
		return BlitterReadWord(address, who);
	else if (address == 0xF00050)
		return tomPITPrescaler;
	else if (address == 0xF00052)
		return tomPITDivider;
...
//
void TOMWriteByte(uint32 address, uint8 data, uint32 who/*=UNKNOWN*/)
{
//???Is this needed???
// Perhaps on the writes--32-bit writes that is! And masked with FF7FFF...
	...
	{
		OPWriteByte(address, data, who);
		return;
	}
	else if ((address >= 0xF02200) && (address < 0xF022A0))
	{
		BlitterWriteByte(address, data, who);
		return;
	}
	else if (address == 0xF00050)
	{
		tomPITPrescaler = (tomPITPrescaler & 0x00FF) | (data << 8);
		tomPITPrescalerCounter = tomPITPrescaler;
		//TOMResetPIT();
		return;
	}
	...
	{
		tomPITPrescaler = (tomPITPrescaler & 0xFF00) | data;
		tomPITPrescalerCounter = tomPITPrescaler;
		//TOMResetPIT();
		return;
	}
	...
	{
		tomPITDivider = (tomPITDivider & 0x00FF) | (data << 8);
		tomPITDividerCounter = tomPITDivider;
		//TOMResetPIT();
		return;
	}
	...
	{
		tomPITDivider = (tomPITDivider & 0xFF00) | data;
		tomPITDividerCounter = tomPITDivider;
		//TOMResetPIT();
		return;
	}
	else if ((address >= 0xF00400) && (address <= 0xF007FF))	// CLUT (A & B)
	{
		...
		uint32 offset = address & 0x5FF;		// Mask out $F00600 (restrict to $F00400-5FF)
		tom_ram_8[offset] = data, tom_ram_8[offset + 0x200] = data;
	}

	tom_ram_8[address & 0x3FFF] = data;
}

//
// TOM word access (write)
//
void TOMWriteWord(uint32 address, uint16 data, uint32 who/*=UNKNOWN*/)
{
//???Is this needed???
	address &= 0xFF3FFF;
...
#endif
if (address == (0xF00000 + MEMCON1))
	WriteLog("TOM: Memory Configuration 1 written by %s: %04X\n", whoName[who], data);
if (address == (0xF00000 + MEMCON2))
...
/*	else if ((address >= 0xF00000) && (address < 0xF00002))
	{
		TOMWriteByte(address, data >> 8);
		TOMWriteByte(address + 1, data & 0xFF);
	}*/
	else if ((address >= 0xF00010) && (address < 0xF00028))
	{
		OPWriteWord(address, data, who);
		return;
	}
	else if (address == 0xF00050)
	{
	...
	else if (address == 0xF000E0)	// INT1
	{
		// Enable interrupts, use only 'enable' bits
		tomIntControl = tomIntControl | (data & TOM_INT04_ENABLE_FLAGS);
		// Clear interrupts, use only 'clear' bits
		tomIntPending = tomIntPending & ~((data & TOM_INT04_CLEAR_FLAGS) >> 8);

#ifdef TOM_DEBUG
		if (data & TOM_INT0_ENABLE)
			WriteLog("TOM: VBLANK interrupt enabled.\n");
		if (data & TOM_INT1_ENABLE)
			WriteLog("TOM: GPU interrupt enabled.\n");
		if (data & TOM_INT2_ENABLE)
			WriteLog("TOM: HBLANK/OP-FLAG interrupt enabled.\n");
		if (data & TOM_INT3_ENABLE)
			WriteLog("TOM: TIMER interrupt enabled.\n");
		if (data & TOM_INT4_ENABLE)
			WriteLog("TOM: JERRY interrupt enabled.\n");

		if (data & TOM_INT0_CLEAR)
			WriteLog("TOM: VBLANK interrupt cleared.\n");
		if (data & TOM_INT1_CLEAR)
			WriteLog("TOM: GPU interrupt cleared.\n");
		if (data & TOM_INT1_CLEAR)
			WriteLog("TOM: HBLANK/OP-FLAG interrupt cleared.\n");
		if (data & TOM_INT3_CLEAR)
			WriteLog("TOM: TIMER interrupt cleared.\n");
		if (data & TOM_INT4_CLEAR)
			WriteLog("TOM: JERRY interrupt cleared.\n");
#endif	// TOM_DEBUG
		return;
	}
	else if ((address >= 0xF02200) && (address <= 0xF0229F))
	{
		BlitterWriteWord(address, data, who);
		return;
	}
	else if ((address >= 0xF00400) && (address <= 0xF007FE))	// CLUT (A & B)
	{
		// Writing to one CLUT writes to the other
		uint32 offset = address & 0x5FF;		// Mask out $F00600 (restrict to $F00400-5FF)
...
{
	// Timer is only running if prescaler != 0
	if (tomPITPrescaler)
	{
		// Decrement prescaler counter
		tomPITPrescalerCounter--;
		if (tomPITPrescalerCounter == 0)
		{
			// Reinitialize prescaler counter
			tomPITPrescalerCounter = tomPITPrescaler + 1;
			// Decrement divider counter
			tomPITDividerCounter--;
			if (tomPITDividerCounter == 0)
			{
				// Reinitialize divider counter
				tomPITDividerCounter = tomPITDivider + 1;

				// Generate TIMER interrupts, does "IRQ enabled" checking
				GPUSetIRQLine(GPU_IRQ_TIMER);
				TOMSetIRQLine(TOM_IRQ_TIMER);	// Generates 68k interrupt
			}
		}
	}
}
