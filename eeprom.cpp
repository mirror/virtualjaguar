...
void eeprom_init(void)
{
	...
	FILE * jerry_ee_fp = fopen(eeprom_filename, "rb");		// Open for reading in binary mode
	if (jerry_ee_fp)
	{
...
void EEPROMSave(void)
{
	FILE * jerry_ee_fp = fopen(eeprom_filename, "wb");		// Open for writeing in binary mode
...
uint8 eeprom_byte_read(uint32 address)
{
	switch (address)
	{
	case 0xF14001:
...
uint16 eeprom_word_read(uint32 address)
{
	return ((uint16)eeprom_byte_read(address + 0) << 8) | eeprom_byte_read(address + 1);
}

void eeprom_byte_write(uint32 address, uint8 data)
{
	switch (address)
	{
	case 0xF14001:
...
void eeprom_word_write(uint32 address, uint16 data)
{
	eeprom_byte_write(address+0, (data >> 8) & 0xFF);
	eeprom_byte_write(address+1, data & 0xFF);
}
