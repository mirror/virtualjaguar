...
uint8 OPReadByte(uint32 address, uint32 who/*=UNKNOWN*/)
{
	uint32 offset = address & 0x3F;
	return objectp_ram[offset];
}

uint16 OPReadWord(uint32 address, uint32 who/*=UNKNOWN*/)
{
	uint32 offset = address & 0x3F;
	return GET16(objectp_ram, offset);
}

void OPWriteByte(uint32 address, uint8 data, uint32 who/*=UNKNOWN*/)
{
	uint32 offset = address & 0x3F;
	objectp_ram[offset] = data;
}

void OPWriteWord(uint32 address, uint16 data, uint32 who/*=UNKNOWN*/)
{
	uint32 offset = address & 0x3F;
...
//This seems more likely...
			...
			if (p0 & 0x08)
			{
				// Generate OP-FLAG interrupt, does "IRQ enabled" checking
				TOMSetIRQLine(TOM_IRQ_OPFLAG);	// Generates 68k interrupt
			}
