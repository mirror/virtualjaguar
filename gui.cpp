...
extern int threadFunction_Scanline(void);
...
void Menu::HandleMouseMove(uint32 x, uint32 y)
{
	...
	if (menuChosen != -1)		// No menu -> no items
	{
...
/*	uint32 * overlayPix = (uint32 *)sdlemuGetOverlayPixels();
	...
	ResetJaguar();		// Start without filename
	return(true);
	// Finito
...
WriteLog("GUI: Bypassing GUI since ROM passed in on command line...\n");
		...
		//memcpy(jaguar_mainRam + 0x100000, jaguar_mainRom, jaguarRomSize);
		//jaguarRunAddress = 0x100000;
//		memcpy(jaguar_mainRam + 0x4000, jaguar_mainRom, jaguarRomSize);
//		jaguarRunAddress = 0x4000;
		//memcpy(jaguar_mainRam + 0x50000, jaguar_mainRom, jaguarRomSize);
		//jaguarRunAddress = 0x50000;
...
showGUI = true;	
		// Some QnD GUI stuff here...
		if (transparency)		//if (showGUI)
		{
			//FillScreenRectangle(overlayPixels, 8, 1*FONT_HEIGHT, 128, 5*FONT_HEIGHT, 0x00000000);
			//FillScreenRectangle(overlayPixels, 8, 1*FONT_HEIGHT, 128, 1*FONT_HEIGHT, 0x00000000);
			//extern uint32 gpu_pc, dsp_pc;
			//extern bool butch_enabled;
			extern s_CD_POSITION_MSF CDReadPositionMSF;

			//if (!butch_enabled)
			//	transparency = 0;		// Clear the string

			DrawString2(overlayPixels, 8, 1*FONT_HEIGHT, 0x001FFF3F, transparency, "CD: %02d:%02d.%02d", CDReadPositionMSF.Minutes, CDReadPositionMSF.Seconds, CDReadPositionMSF.Frames);
			//uint32 m68kPC = m68k_get_reg(NULL, M68K_REG_PC);
			//DrawString(overlayPixels, 8, 1*FONT_HEIGHT, false, "68K PC: %08X", m68kPC);
			//DrawString(overlayPixels, 8, 2*FONT_HEIGHT, false, "GPU PC: %08X", gpu_pc);
			//DrawString(overlayPixels, 8, 3*FONT_HEIGHT, false, "DSP PC: %08X", dsp_pc);
			//DrawString(overlayPixels, 8, 5*FONT_HEIGHT, false, "%u FPS", framesPerSecond);
			//DrawString2(overlayPixels, 8, 2*FONT_HEIGHT, 0x001FFF3F, transparency, "%u FPS", framesPerSecond);
		}
...
*/
		...
		//WriteLog("Frame done2: Ticks ms: %d\n", SDL_GetTicks() - FrameStart);
		//framesPerSecond = 1000 / (SDL_GetTicks() - FrameStart);
		//uint32 TimePerFrame = SDL_GetTicks() - FrameStart;
		//WriteLog("TimePerFrame: %d\n", TimePerFrame);
		//if (SDL_GetTicks() - elapsedTicks > 250)
		//	elapsedTicks += 250, framesPerSecond = frameCount * 4, frameCount = 0;
	}
...
//Hardwired, bleh... !!! FIX !!!
uint32 width = 55 * FONT_WIDTH, height = 19 * FONT_HEIGHT;
...
//	window->AddElement(new Text(208, 8+0*FONT_HEIGHT, buf, 0xFF3030FF, 0xFF000000));
	...
	window->AddElement(new Text(232, 8+4*FONT_HEIGHT+5, buf, 0xFF3030FF, 0xFF000000));
	...
	window->AddElement(new Text(16, 8+5*FONT_HEIGHT, "Stephan Kapfer"));
	window->AddElement(new Text(8, 8+7*FONT_HEIGHT, "Testers:"));
	window->AddElement(new Text(16, 8+8*FONT_HEIGHT, "Guruma"));
	window->AddElement(new Text(8, 8+10*FONT_HEIGHT, "Thanks go out to:"));
	window->AddElement(new Text(16, 8+11*FONT_HEIGHT, "Aaron Giles for the original CoJag"));
	window->AddElement(new Text(16, 8+12*FONT_HEIGHT, "David Raingeard for the original VJ"));
	window->AddElement(new Text(16, 8+13*FONT_HEIGHT, "Karl Stenerud for his Musashi 68K emu"));
	window->AddElement(new Text(16, 8+14*FONT_HEIGHT, "Sam Lantinga for his amazing SDL libs"));
	window->AddElement(new Text(16, 8+15*FONT_HEIGHT, "Ryan C. Gordon for VJ's web presence"));
	window->AddElement(new Text(16, 8+16*FONT_HEIGHT, "Curt Vendel for various Jaguar goodies"));
