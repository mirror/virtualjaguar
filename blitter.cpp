...
uint8 BlitterReadByte(uint32 address, uint32 who/*=UNKNOWN*/)
{
...
//Crappy!
uint16 BlitterReadWord(uint32 address, uint32 who/*=UNKNOWN*/)
{
	return ((uint16)BlitterReadByte(address, who) << 8) | (uint16)BlitterReadByte(address+1, who);
}

//Crappy!
uint32 BlitterReadLong(uint32 address, uint32 who/*=UNKNOWN*/)
{
	return (BlitterReadWord(address, who) << 16) | BlitterReadWord(address+2, who);
}

void BlitterWriteByte(uint32 address, uint8 data, uint32 who/*=UNKNOWN*/)
{
/*if (offset & 0xFF == 0x7B)
...
void BlitterWriteWord(uint32 address, uint16 data, uint32 who/*=UNKNOWN*/)
{
...
//#endif
	...
	BlitterWriteByte(address + 0, data >> 8, who);
	BlitterWriteByte(address + 1, data & 0xFF, who);

	if ((address & 0xFF) == 0x3A)
...
void BlitterWriteLong(uint32 address, uint32 data, uint32 who/*=UNKNOWN*/)
{
/*if (((offset & 0xFF) >= PATTERNDATA) && ((offset & 0xFF) < PATTERNDATA + 8))
{
...
//#endif
	...
	BlitterWriteWord(address + 0, data >> 16, who);
	BlitterWriteWord(address + 2, data & 0xFFFF, who);
}
