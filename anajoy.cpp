...
void anajoy_byte_write(uint32 address, uint8 data)
{
	anajoy_ram[address & 0x01]=data;
}

void anajoy_word_write(uint32 address, uint16 data)
{
	uint32 offset = address & 0x01;
	anajoy_ram[offset+0]=(data>>8)&0xff;
	anajoy_ram[offset+1]=data&0xff;
}

uint8 anajoy_byte_read(uint32 address)
{
...
uint16 anajoy_word_read(uint32 address)
{
	uint16 data=anajoy_byte_read((address+0) & 0x01);
	data<<=8;
	data|=anajoy_byte_read((address+1) & 0x01);
	return(data);
}
