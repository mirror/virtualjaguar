...
uint8 joystick_byte_read(uint32 address)
{
//	extern bool hardwareTypeNTSC;
	uint32 offset = address & 0x03;
...
uint16 joystick_word_read(uint32 address)
{
	return ((uint16)joystick_byte_read((address + 0) & 0x03) << 8) | joystick_byte_read((address + 1) & 0x03);
}

void joystick_byte_write(uint32 address, uint8 data)
{
	joystick_ram[address & 0x03] = data;
}

void joystick_word_write(uint32 address, uint16 data)
{
	uint32 offset = address & 0x03;
